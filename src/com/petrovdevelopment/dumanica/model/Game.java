...
public class Game {
	...
	public void update(String guessedLetter) {
		if (!mGameFinished) {
			...
			mLastLetterGuessCorrect = getCurrentWordVM().isLastLetterGuessCorrect();
			
			mPoints = mPoints + getCurrentWordVM().getPointsForLastGuess();

			if (getCurrentWordVM().isFinished()) {
				if (getCurrentWordVM().isSuccess())
					mGuessedWordsCount++;
	...
	private WordVM removeNextWordVM() {
		if (mWordViewModels.size() > 0)
			return mWordViewModels.remove(0);
		else
			return null;
	}
	...
	public int getWordsLeft() {
		return mWordViewModels.size() + 1; //+1 because of the current word which is not part of the list
	}
